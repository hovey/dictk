...
```bash
uv run pdoc dictk dictk.imaging dictk.cli -o docs/api
```
...
below), so it's picked up automatically. The other submodules (`imaging`,
...
```bash
uv run pdoc dictk dictk.imaging dictk.cli   # live preview, serves on localhost
```
