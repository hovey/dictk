...
architecture"](#cicd-architecture) below), so the root is a small landing
page linking to `main/` and `dev/`, generated by
`.github/scripts/render_landing.py` and regenerated on every deploy from
whichever branch ran most recently:
